...
// ── Course Detail ─────────────────────────────────────────
function CourseDetailPage({ onNavigate, params = {}, user, onLogin }) {
  const { data } = window.CIAL;
  ...
  function handleBuy() {
    ...
    setPurchasing(true);
    setTimeout(() => { setPurchasing(false); setPurchased(true); data.enrolledIds.push(course.id); }, 1400);
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#faf9ff' }}>
      {/* Hero */}
      <div style={{ background: 'linear-gradient(135deg,#1e0a3c,#3b1a7a)', padding: 'clamp(32px,5vw,56px) 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr clamp(280px,36%,380px)', gap: 40, alignItems: 'start' }} className="detail-hero">
          <div>
          ...
          {/* Purchase card */}
          <PurchaseCard course={course} discount={discount} isEnrolled={isEnrolled} purchased={purchased} purchasing={purchasing} onBuy={handleBuy} onNavigate={onNavigate} user={user} />
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 24px', display: 'grid', gridTemplateColumns: '1fr clamp(280px,36%,380px)', gap: 40, alignItems: 'start' }} className="detail-body">
          ...
          <Section title={`Contenido del curso · ${course.lessons} clases · ${course.duration}`}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {course.modules.map((mod, i) => (
                <div key={i} style={{ border: '1.5px solid #e2d9f7', borderRadius: 12, overflow: 'hidden' }}>
                  <button onClick={() => setOpenModule(openModule === i ? null : i)}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', background: openModule === i ? '#f5f3ff' : '#fff', border: 'none', cursor: 'pointer', fontFamily: 'inherit', transition: 'background .15s' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
                      ...
                      <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="#94a3b8" strokeWidth="2" strokeLinecap="round" style={{ transform: openModule === i ? 'rotate(180deg)' : 'none', transition: 'transform .2s' }}><path d="M4 6l4 4 4-4"/></svg>
                    </div>
                  </button>
                  {openModule === i && (
                    <div style={{ padding: '12px 18px', background: '#faf9ff', borderTop: '1px solid #f0ebfd' }}>
                      {Array.from({ length: Math.min(mod.lessons, 4) }).map((_, j) => (
                        <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderBottom: j < Math.min(mod.lessons, 4) - 1 ? '1px solid #f0ebfd' : 'none' }}>
                          <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><circle cx="9" cy="9" r="8" fill="#ede9fe"/><path d="M7 6.5l5 2.5-5 2.5V6.5z" fill="#7c3aed"/></svg>
                          <span style={{ fontSize: 13, color: '#374151' }}>Clase {j + 1}: {mod.title} — parte {j + 1}</span>
                          {(j === 0 && course.free) && <span style={{ marginLeft: 'auto', fontSize: 11, color: '#059669', fontWeight: 600 }}>Vista previa</span>}
                        </div>
                      ))}
                      {mod.lessons > 4 && <div style={{ fontSize: 12, color: '#94a3b8', paddingTop: 10 }}>+ {mod.lessons - 4} clases más</div>}
                    </div>
                  )}
                </div>
              ))}
        ...
        {/* Purchase card desktop repeat (hidden on mobile via class) */}
        <div className="detail-card-desktop">
          <PurchaseCard course={course} discount={discount} isEnrolled={isEnrolled} purchased={purchased} purchasing={purchasing} onBuy={handleBuy} onNavigate={onNavigate} user={user} sticky />
        </div>
      </div>
    </div>
  );
